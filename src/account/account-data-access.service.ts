...
export class AccountDataAccessService {
  ...
  public async getAllDuesTypeAndAmountOfClubMember(userId: number, memberId: number, year: string) {
    ...
    const memberDuesPaymentPerYearMap = memberDuesPayment.filter((payment) => payment.lionYear === year).reduce((acc, curr) => {
  ...
  private async getAllDuesTypeNameAndPayments(userId: number, year: string): Promise<DuesQueryResult[]> {
  ...
  private async getAllIncomeTypeNameAndAmount(userId: number, year: string): Promise<IncomeOrExpenseQueryResult[]> {
  ...
  private async getAllExpenseTypeNameAndAmount(userId: number, year: string): Promise<IncomeOrExpenseQueryResult[]> {
        ...
        if (incomeType) {
          if (!acc[incomeType]) {
            ...
            acc[incomeType].map((income) => {
              if(income.incomeName !== incomeName) return acc[incomeType].push({
                incomeName: incomeName,
                totalIncome: curr.amount
              })
        ...
        if (expenseType) {
          if (!acc[expenseType]) {
            ...
            acc[expenseType].map((expense) => {
              if(expense.expenseName !== expenseName) return acc[expenseType].push({
                expenseName: expenseName,
                totalexpense: curr.amount
              })
