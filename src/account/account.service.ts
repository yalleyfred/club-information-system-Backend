...
export class AccountService {
  ...
  public async createAdminAccount(userId: number, year: string) {
    try {
      const duesInfo =
        await this.accountDataAccessService.getAllDuesTypeAndAmountOfClub(
          userId,
          year,
        );
      const incomeInfo =
        await this.accountDataAccessService.getAllIncomeTypeAndAmountOfClub(
          userId,
          year,
        );
      const expenseInfo =
        await this.accountDataAccessService.getAllExpenseTypeAndAmountOfClub(
          userId,
          year,
        );
  ...
  public async createMemberAccountReport(
    ...
    try {
      const memberData: Member = await this.accountDataAccessService.getAllDuesTypeAndAmountOfClubMember(userId, memberId, year);
