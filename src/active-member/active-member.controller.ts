...
export class ActiveMemberController {
    ...
    @HttpCode(201)
    @Post()
    createActiveMember(@GetUser('id') userId:number, @Body() dto:CreateActiveMemberDto) {
        return this.activeMemberService.createActiveMember(userId, dto)
    }

    @HttpCode(200)
    @Get()
    getActiveMembers(@GetUser('id') userId:number) {
        return this.activeMemberService.getActiveMembers(userId)
    }

    @HttpCode(200)
    @Get(':id')
    getActiveMemberById(@GetUser('id') userId:number, @Param('id', ParseIntPipe) dropedId:number) {
        return this.activeMemberService.getActiveMemberById(userId, dropedId)
    }

    @HttpCode(200)
    @Patch(':id')
    updateActiveMember(@GetUser('id') userId:number, @Param('id', ParseIntPipe) dropedId:number, @Body() dto: EditActiveMemberDto) {
        return this.activeMemberService.updateActiveMember(userId, dropedId, dto)
    }

    @HttpCode(200)
    @Delete(':id')
    removeActiveMemberById(@GetUser('id') userId:number, @Param('id', ParseIntPipe) dropedId:number) {
